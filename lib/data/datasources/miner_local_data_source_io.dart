...
class MinerLocalDataSource {
  ...
  Future<MinerPoolConfigSnapshot?> fetchPoolConfig(
    ...
    try {
      ...
      if (snapshot == null) {
        _log(
          'poolcfg.parse_failed ip=$ip body=${_singleLineSnippet(raw.body, 200)}',
        );
      }
      return snapshot;
    } catch (_) {
  ...
  Future<LedToggleResult> applyPoolConfig(
    ...
    final keepPowerBody = '_bb_nonetwork_run=${snapshot.keepPower ? '1' : '0'}';
    final keepPowerResult = await _postAction(
      ip: ip,
      credential: credential,
      path: '/cgi-bin/set_nonetworkrun_mode.cgi',
      successMessage: 'Non-network run config sent.',
      formBody: keepPowerBody,
    );
    if (!keepPowerResult.success) {
      return keepPowerResult;
    }
    final body = _buildPoolConfigFormBody(snapshot);
    return _postAction(
      ip: ip,
      credential: credential,
  ...
  MinerRuntime _parseRuntime(String body, String ip) {
    ...
    return MinerRuntime(
      ...
      chains: _parseChains(data['chains'] ?? body),
      logSnippet: '--',
  ...
  List<MinerChainStatus> _parseChains(dynamic raw) {
    return parseVolcMinerChains(raw)
        .map(
          (chain) => MinerChainStatus(
            index: chain.index,
  ...
  String _buildPoolConfigFormBody(MinerPoolConfigSnapshot snapshot) {
    final normalized = [...snapshot.poolSlots]
    ...
    for (final slot in normalized) {
      ...
      fields['_bb_pool${slot.slotNo}pw'] =
          (snapshot.slotPasswords[slot.slotNo] ?? '').trim();
    }
    fields['_bb_nobeeper'] = snapshot.noBeeper.trim();
    fields['_bb_notempoverctrl'] = (!snapshot.stopRunOnOverTemp).toString();
    fields['_bb_fan_customize_switch'] = snapshot.fanCustomizeEnabled
        .toString();
    fields['_bb_fan_customize_value_front'] = snapshot.fanSpeedFront.trim();
    fields['_bb_fan_customize_value_back'] = snapshot.fanSpeedBack.trim();
    fields['_bb_freq'] = snapshot.frequency.trim();
    fields['_bb_coin_type'] = snapshot.algorithm.trim();
    fields['_bb_runmode'] = snapshot.runningMode.trim();
    fields['_bb_voltage_customize_value'] = snapshot.voltageCustomizeValue
        .trim();
    fields['_bb_ema'] = snapshot.memParam.trim();
    fields['_bb_debug'] = snapshot.debugEnabled ? 'true' : 'false';
  ...
  MinerPoolConfigSnapshot? _parsePoolConfigSnapshot(String body) {
    ...
    final configPools = _extractPools(map, body);
    final runningMode = _firstNonEmptyValue([
      _readConfigValue(map, body, const [
        'runmode',
        'running_mode',
        '_bb_runmode',
      ]),
      _readConfigValue(map, _decodeEscapedText(body), const ['runmode']),
    ]);
    final algorithm = _firstNonEmptyValue([
      _readConfigValue(map, body, const ['coin-type', '_bb_coin_type']),
      'ltc',
    ]);
    final frequency = _firstNonEmptyValue([
      _readConfigValue(map, body, const ['freq', '_bb_freq']),
      '2200',
    ]);
    final memParam = _firstNonEmptyValue([
      _readConfigValue(map, body, const [
        'sram-voltage',
        '_bb_ema',
    ...
    final voltageValue = _firstNonEmptyValue([
      _readConfigValue(map, body, const [
        'voltage',
        '_bb_voltage_customize_value',
      ]),
    ...
    final keepPower = _isTruthy(
      _readConfigValue(map, body, const ['keepower', '_bb_nonetwork_run']),
    );
    final fanCustomizeEnabled = _isTruthy(
      _readConfigValue(map, body, const [
        'fan-ctrl',
        '_bb_fan_customize_switch',
      ]),
    );
    final fanSpeedFront = _readConfigValue(map, body, const [
      'fan-pwm-front',
      '_bb_fan_customize_value_front',
    ]);
    final fanSpeedBack = _readConfigValue(map, body, const [
      'fan-pwm-back',
      '_bb_fan_customize_value_back',
    ]);
    final noBeeper = _readConfigValue(map, body, const [
      'nobeeper',
      '_bb_nobeeper',
    ]);
    final noTempOverCtrl = _readConfigValue(map, body, const [
      'notempoverctrl',
      '_bb_notempoverctrl',
    ]);
    final debugEnabled = _extractDebugEnabled(map, body);
    final stopRunOnOverTemp = noTempOverCtrl.isEmpty
        ? true
        : !_isTruthy(noTempOverCtrl);
    final normalizedRunningMode = runningMode.isEmpty ? '0' : runningMode;
    for (var slotNo = 1; slotNo <= 3; slotNo++) {
      final Map<String, dynamic>? pool = configPools.length >= slotNo
          ? configPools[slotNo - 1]
          : null;
      final poolUrl =
          _normalizePoolValue(pool?['url']) ??
          _readConfigValue(map, body, [
            '_bb_pool${slotNo}url',
            'pool${slotNo}url',
            'pool$slotNo.url',
          ]);
      final workerCode =
          _normalizePoolValue(pool?['user']) ??
          _readConfigValue(map, body, [
            '_bb_pool${slotNo}user',
            'pool${slotNo}user',
            'pool$slotNo.user',
          ]);
      final password =
          _normalizePoolValue(pool?['pass']) ??
          _readConfigValue(map, body, [
            '_bb_pool${slotNo}pw',
            'pool${slotNo}pw',
            'pool$slotNo.pw',
    ...
    return MinerPoolConfigSnapshot(
      poolSlots: slots,
      slotPasswords: passwords,
      runningMode: normalizedRunningMode,
      algorithm: algorithm,
      keepPower: keepPower,
      stopRunOnOverTemp: stopRunOnOverTemp,
      fanCustomizeEnabled: fanCustomizeEnabled,
      fanSpeedFront: fanSpeedFront,
      fanSpeedBack: fanSpeedBack,
      frequency: frequency,
      voltageCustomizeValue: voltageValue,
      memParam: memParam,
  ...
  List<Map<String, dynamic>> _extractPools(
    Map<String, dynamic>? map,
    String rawBody,
  ) {
    if (map == null) {
      return _extractPoolsFromRaw(rawBody);
    }
    final rawCfgs = map['cfgs'];
    dynamic cfgsDecoded = rawCfgs;
    if (rawCfgs is String && rawCfgs.trim().isNotEmpty) {
      try {
        cfgsDecoded = jsonDecode(rawCfgs);
      } catch (_) {
        cfgsDecoded = null;
      }
    }
    if (cfgsDecoded is! List || cfgsDecoded.isEmpty) {
      return _extractPoolsFromRaw(rawBody);
    }
    final first = cfgsDecoded.first;
    if (first is! Map) {
      return _extractPoolsFromRaw(rawBody);
    }
    final pools = first['pools'];
    if (pools is! List) {
      return _extractPoolsFromRaw(rawBody);
    }
    return pools
  ...
  List<Map<String, dynamic>> _extractPoolsFromRaw(String rawBody) {
    final normalized = _decodeEscapedText(rawBody);
    final matches = RegExp(
      r'"url"\s*:\s*"((?:\\.|[^"])*)"\s*,\s*"user"\s*:\s*"((?:\\.|[^"])*)"\s*,\s*"pass"\s*:\s*"((?:\\.|[^"])*)"',
      multiLine: true,
      dotAll: true,
    ...
    return matches
        .map(
          (match) => <String, dynamic>{
            'url': _decodeEscapedText(match.group(1) ?? ''),
            'user': _decodeEscapedText(match.group(2) ?? ''),
            'pass': _decodeEscapedText(match.group(3) ?? ''),
          },
        )
        .toList(growable: false);
  }

  String? _normalizePoolValue(dynamic value) {
    if (value == null) {
      return null;
    }
    final normalized = _decodeEscapedText('$value'.trim());
    ...
    try {
      ...
      if (rawData is String && rawData.trim().isNotEmpty) {
        ...
        try {
          ...
          final normalized = _decodeEscapedText(cleaned);
  ...
  String _firstNonEmptyValue(List<String> values) {
  ...
  bool _extractDebugEnabled(Map<String, dynamic>? map, String rawBody) {
    final dynamic rawDebug = map?['debug'];
    if (rawDebug is Map && rawDebug['bb_debug_enable'] != null) {
      return _isTruthy('${rawDebug['bb_debug_enable']}');
    }
    if (rawDebug is String && rawDebug.trim().isNotEmpty) {
      final normalized = _decodeEscapedText(rawDebug);
      final match = RegExp(
        r'"bb_debug_enable"\s*:\s*(true|false|"true"|"false"|1|0)',
        caseSensitive: false,
      ).firstMatch(normalized);
      if (match != null) {
        return _isTruthy(match.group(1)?.replaceAll('"', '') ?? '');
      }
    }
    final match = RegExp(
      r'"bb_debug_enable"\s*:\s*(true|false|"true"|"false"|1|0)',
      caseSensitive: false,
    ).firstMatch(_decodeEscapedText(rawBody));
    if (match != null) {
      return _isTruthy(match.group(1)?.replaceAll('"', '') ?? '');
    }
    return false;
