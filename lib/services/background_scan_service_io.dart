...
AutoScanProgress? _windowsLatestProgress;
DateTime? _windowsLastAutoScanAt;
DateTime? _windowsLastAutoScanAttemptAt;
DateTime? _windowsNextAutoScanAt;
String _windowsScheduleSignature = '';
AppSettings? _windowsForegroundSettings;
List<ScanView> _windowsForegroundViews = const [];
String? _windowsForegroundMinerAuthPassword;
String? _windowsForegroundPoolSearchPassword;
IsarLocalDataSource? _windowsForegroundLocalDataSource;
final StreamController<Map<String, dynamic>?> _localEvents =
    StreamController<Map<String, dynamic>?>.broadcast();

void _logWindowsAutoScan(String message) {
  if (!Platform.isWindows) {
    return;
  }
  debugPrint('[WindowsAutoScan] $message');
}
...
class BackgroundScanService {
  ...
  static final bool _supportsBackgroundService =
      Platform.isAndroid || Platform.isIOS;
  static final bool _skipPlatformInitialization = Platform.isWindows;

  static Future<void> initialize() async {
    if (_skipPlatformInitialization) {
      return;
    }
    ...
    const windowsInit = WindowsInitializationSettings(
      appName: 'VolcMiner',
      appUserModelId: 'com.ronganbai.volcminer',
      guid: 'f1bb4c26-3cf0-48ab-9854-219f8b0ab8d7',
    );
  ...
  static Future<void> sync({required bool enabled}) async {
    if (_skipPlatformInitialization) {
      _logWindowsAutoScan('sync enabled=$enabled');
      if (!enabled) {
        _backgroundTimer?.cancel();
        _backgroundTimer = null;
        _windowsScheduleSignature = '';
        _windowsLastAutoScanAttemptAt = null;
        _windowsNextAutoScanAt = null;
        _logWindowsAutoScan('sync disabled -> timer cancelled');
  ...
  static Stream<Map<String, dynamic>?> on(String event) {
    if (_skipPlatformInitialization) {
      if (event != 'scanUpdated') {
        return const Stream.empty();
      }
      return _localEvents.stream;
    }
    if (!_supportsBackgroundService) {
      return const Stream.empty();
    }
  ...
  static void updateWindowsForegroundState({
    ...
    _windowsForegroundSettings = settings;
    _windowsForegroundViews = List<ScanView>.unmodifiable(views);
    _windowsForegroundMinerAuthPassword = minerAuthPassword;
    _windowsForegroundPoolSearchPassword = poolSearchPassword;
    _windowsForegroundLocalDataSource = localDataSource;
    _logWindowsAutoScan(
      'foreground state updated views=${views.length} autoRefresh=${settings.autoRefreshEnabled} interval=${settings.refreshIntervalSec}s window=${settings.autoScanStartMinute}-${settings.autoScanStopMinute} concurrency=${settings.scanConcurrency}',
    );
  }
  ...
  static Future<AutoScanProgress> getAutoScanProgress() async {
    if (_skipPlatformInitialization && _windowsLatestProgress != null) {
      return _windowsLatestProgress!;
    }
    final prefs = await SharedPreferences.getInstance();
    await prefs.reload();
  ...
  static Future<DateTime?> getLastAutoScanAt() async {
    if (_skipPlatformInitialization && _windowsLastAutoScanAt != null) {
      return _windowsLastAutoScanAt;
    }
    final prefs = await SharedPreferences.getInstance();
    await prefs.reload();
  ...
  static Future<DateTime?> getLastAutoScanAttemptAt() async {
    if (_skipPlatformInitialization && _windowsLastAutoScanAttemptAt != null) {
      return _windowsLastAutoScanAttemptAt;
    }
    final prefs = await SharedPreferences.getInstance();
    await prefs.reload();
  ...
  static Future<DateTime?> getNextAutoScanAtStored() async {
    if (_skipPlatformInitialization && _windowsNextAutoScanAt != null) {
      return _windowsNextAutoScanAt;
    }
    final prefs = await SharedPreferences.getInstance();
    await prefs.reload();
...
Future<void> _onServiceStart(ServiceInstance service) async {
  ...
  if (service is AndroidServiceInstance) {
    ...
    service.setForegroundNotificationInfo(
      ...
      content: 'Background auto-refresh is active',
    );
  }
...
Future<void> _restartWindowsSchedule() async {
  _logWindowsAutoScan('restart schedule requested');
  final context = await _loadBackgroundContext();
  if (context == null) {
    _logWindowsAutoScan('restart schedule aborted: context unavailable');
    return;
  }
  if (!context.settings.autoRefreshEnabled) {
    _backgroundTimer?.cancel();
    _backgroundTimer = null;
    _windowsScheduleSignature = '';
    _logWindowsAutoScan('restart schedule aborted: auto refresh disabled');
    return;
  }
  ...
  final scheduleSignature = [
    context.settings.autoRefreshEnabled,
    intervalSeconds,
    context.settings.autoScanStartMinute,
    context.settings.autoScanStopMinute,
    context.settings.scanConcurrency,
    context.settings.minerUsername,
    context.minerAuthPassword,
    context.settings.poolSearchUsername,
    context.poolSearchPassword,
    context.views.map((view) => view.id).join(','),
  ].join('|');
  if (_backgroundTimer != null &&
      _backgroundTimer!.isActive &&
      scheduleSignature == _windowsScheduleSignature) {
    final currentNextAt = _windowsNextAutoScanAt;
    if (currentNextAt == null || !currentNextAt.isBefore(DateTime.now())) {
      _logWindowsAutoScan(
        'restart schedule skipped: existing timer active next=${currentNextAt?.toIso8601String() ?? 'null'}',
      );
      return;
    }
  }
  ...
  _backgroundTimer?.cancel();
  _windowsScheduleSignature = scheduleSignature;
  final now = DateTime.now();
  final nextAllowed = _alignToAllowedWindow(
    context.settings,
    now.add(Duration(seconds: intervalSeconds)),
  ...
  await _markNextAutoScanAt(nextAllowed);
  _logWindowsAutoScan(
    'schedule armed interval=${intervalSeconds}s shouldRunNow=$shouldRunNow next=${nextAllowed.toIso8601String()} views=${context.views.length}',
  );

  if (shouldRunNow) {
    _logWindowsAutoScan('schedule triggering immediate run');
    unawaited(_runBackgroundScan(null));
  }

  _backgroundTimer = Timer.periodic(Duration(seconds: intervalSeconds), (_) {
    _logWindowsAutoScan('periodic timer fired');
    unawaited(_runBackgroundScan(null));
  });
}
...
Future<void> _runBackgroundScan(ServiceInstance? service) async {
  if (_backgroundScanBusy) {
    ...
    _logWindowsAutoScan('run skipped: busy, queued pending rerun');
    return;
  }
  _backgroundScanBusy = true;
  _backgroundRunPending = false;
  final startedAt = DateTime.now();
  _logWindowsAutoScan('run started at ${startedAt.toIso8601String()}');
  ...
  try {
    ...
    final context = await _loadBackgroundContext();
    if (context == null) {
      _logWindowsAutoScan('run aborted: context unavailable');
    ...
    if (!context.settings.autoRefreshEnabled) {
      _logWindowsAutoScan('run skipped: auto refresh disabled');
      await _finishAutoScanLog(
        logId,
        status: 'skipped',
    ...
    if (!_isWithinAutoScanWindow(context.settings, startedAt)) {
      _logWindowsAutoScan('run skipped: outside active time window');
    ...
    if (context.views.isEmpty) {
      _logWindowsAutoScan('run skipped: no scan views');
      await _finishAutoScanLog(
        logId,
        status: 'skipped',
    ...
    if (context.controller.snapshot.knownMinerIpsByScope.isEmpty) {
      _logWindowsAutoScan('run skipped: no known miner IPs');
      await _finishAutoScanLog(
        logId,
        status: 'skipped',
    ...
    _logWindowsAutoScan(
      'run scanning known targets=$totalTargets knownScopes=${context.controller.snapshot.knownMinerIpsByScope.length}',
    );
    await _saveAutoScanProgress(
      AutoScanProgress(
        isRunning: true,
    ...
    try {
      ...
      _logWindowsAutoScan(
        'run success onlineCount=$onlineCount totalTargets=$totalTargets next=${_windowsNextAutoScanAt?.toIso8601String() ?? 'null'}',
      );
      await _finishAutoScanLog(
        logId,
        status: 'success',
      ...
      final eventPayload = {'timestamp': DateTime.now().toIso8601String()};
      service?.invoke('scanUpdated', eventPayload);
      _localEvents.add(eventPayload);
    } finally {
      progressTimer.cancel();
    }
  } catch (e) {
    _logWindowsAutoScan('run failed: $e');
    await _finishAutoScanLog(
      logId,
      status: 'failed',
      note: '$e',
    ...
    if (_backgroundRunPending) {
      _backgroundRunPending = false;
      _logWindowsAutoScan('running queued rerun now');
      unawaited(_runBackgroundScan(service));
    } else {
      _logWindowsAutoScan('run finished');
    }
  }
}
...
Future<_BackgroundContext?> _loadBackgroundContext() async {
  try {
    if (Platform.isWindows &&
        _windowsForegroundSettings != null &&
        _windowsForegroundMinerAuthPassword != null &&
        _windowsForegroundPoolSearchPassword != null &&
        _windowsForegroundLocalDataSource != null) {
      _logWindowsAutoScan(
        'load context from foreground cache views=${_windowsForegroundViews.length}',
      );
      ...
      final controller = ScanController(
        ...
        _windowsForegroundLocalDataSource!,
      );

      return _BackgroundContext(
        controller: controller,
        settings: _windowsForegroundSettings!,
        views: _windowsForegroundViews,
        minerAuthPassword: _windowsForegroundMinerAuthPassword!,
        poolSearchPassword: _windowsForegroundPoolSearchPassword!,
      );
    }
    ...
    _logWindowsAutoScan(
      'load context from storage views=${views.length} autoRefresh=${settings.autoRefreshEnabled}',
    );
    ...
    _logWindowsAutoScan('load context failed: $e');
    _logWindowsAutoScan('$stackTrace');
    return null;
  }
}
...
Future<void> _markAutoScanRan() async {
  final now = DateTime.now();
  _windowsLastAutoScanAt = now;
  final prefs = await SharedPreferences.getInstance();
  await prefs.setString(_lastAutoScanAtKey, now.toIso8601String());
}

Future<void> _markAutoScanAttempted() async {
  final now = DateTime.now();
  _windowsLastAutoScanAttemptAt = now;
  final prefs = await SharedPreferences.getInstance();
...
Future<void> _saveAutoScanProgress(AutoScanProgress progress) async {
  _windowsLatestProgress = progress;
  final prefs = await SharedPreferences.getInstance();
...
Future<void> _showNotification({
  ...
  if (BackgroundScanService._skipPlatformInitialization) {
    return;
  }
